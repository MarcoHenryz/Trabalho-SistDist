...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \texttt{ENTRYPOINT}: Executável principal do container
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{CMD vs ENTRYPOINT}
  \textbf{CMD} — Define o comando padrão, mas pode ser \textbf{substituído}:

  \vspace{0.3em}
  \small
  \texttt{CMD ["python", "app.py"]}

  \texttt{docker run minha-imagem echo "outro"} $\rightarrow$ executa \texttt{echo "outro"}

  \vspace{0.8em}
  \normalsize
  \textbf{ENTRYPOINT} — Define o executável fixo. Argumentos são \textbf{adicionados}:

  \vspace{0.3em}
  \small
  \texttt{ENTRYPOINT ["python", "app.py"]}

  \texttt{docker run minha-imagem --verbose} $\rightarrow$ executa \texttt{python app.py --verbose}
\end{frame}

\begin{frame}
  \frametitle{CMD + ENTRYPOINT juntos}
  Padrão mais comum: ENTRYPOINT para o executável, CMD para argumentos padrão.
  ...
  \vspace{0.5em}
  \begin{itemize}
    \item Sem argumentos: \texttt{python app.py --port 8080}
    \item Com argumentos: \texttt{docker run img --port 3000}

          $\rightarrow$ \texttt{python app.py --port 3000} (CMD substituído)
  \end{itemize}

  \vspace{0.5em}
  \begin{block}{Resumo}
    \texttt{ENTRYPOINT} = o ``verbo'' (o que o container faz)

    \texttt{CMD} = o ``complemento'' (argumentos padrão)
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item \texttt{EXPOSE}, \texttt{USER}, \texttt{CMD}/\texttt{ENTRYPOINT}: Configurar a imagem final
  \end{enumerate}
\end{frame}
